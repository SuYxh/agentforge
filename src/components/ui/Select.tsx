  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      <button
        ...
        disabled={disabled}
        className={`
          w-full h-10 px-3 rounded-lg bg-white/20 dark:bg-white/8 border border-white/15 dark:border-white/8 backdrop-blur-sm text-sm text-left
          ...
          {groupNames.length === 1 && groupNames[0] === '' ? (
            ...
            groupNames.map((groupName, idx) => (
              <div key={groupName || 'default'}>
                {groupName && (
                  <div className="px-3 py-1.5 text-xs font-medium text-muted-foreground uppercase tracking-wider bg-white/10 dark:bg-white/5">
                    {groupName}
                  </div>
                )}
                <div className="py-1">
  ...
  return (
    <button
      ...
      className={`
        ...
        ${isSelected 
          ? 'bg-white/20 dark:bg-white/10 text-primary font-medium' 
