...
export function SkillFileEditor({
  ...
  const loadFiles = useCallback(async () => {
    setIsLoading(true);
    try {
      const result = await skillApi.listLocalFiles(skillId);
      setFiles(result);
      const firstFile =
        result.find(
          (entry) =>
  ...
  useEffect(() => {
    ...
    ).__AGENTFORGE_SKILL_EDITOR_DIRTY = hasAnyUnsaved;

    return () => {
      (
        window as Window & { __AGENTFORGE_SKILL_EDITOR_DIRTY?: boolean }
      ).__AGENTFORGE_SKILL_EDITOR_DIRTY = false;
    };
  ...
  const loadSelectedFileContent = useCallback(
    async (path: string) => {
      ...
      try {
        const result = await skillApi.readLocalFile(skillId, path);
  ...
  const handleNewFile = useCallback(async () => {
    ...
    try {
      ...
      if (dirParts.length > 1) {
        ...
        await skillApi.createLocalDir(skillId, dirPath);
      }
      await skillApi.writeLocalFile(skillId, name, "");
      await loadFiles();
  ...
  const handleNewFolder = useCallback(async () => {
    ...
    if (!name) return;
    try {
      await skillApi.createLocalDir(skillId, name);
      await loadFiles();
  ...
  const handleRenamePath = useCallback(async () => {
    ...
    try {
      await skillApi.renameLocalPath(skillId, renameDialogPath, nextPath);
      setModifiedFiles((prev) => {
  ...
  const handleDeleteFile = useCallback(async () => {
    ...
    try {
      await skillApi.deleteLocalFile(skillId, deleteDialogFile);
  ...
  const handleOpenInExplorer = useCallback(async () => {
    try {
      const repoPath = await skillApi.getRepoPath(skillId);
      if (!repoPath) {
        showToast(t("skill.noLocalRepo", "No local repository found"), "error");
        return;
      }
      // TODO: implement Tauri open path
    } catch (error) {
