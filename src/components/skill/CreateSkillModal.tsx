...
export function CreateSkillModal({ isOpen, onClose }: CreateSkillModalProps) {
  ...
  const handleEnterNativeFullscreen = useCallback(() => {
    ...
    // TODO: implement enter fullscreen via Tauri command
  }, []);

  const handleExitNativeFullscreen = useCallback(() => {
    setIsNativeFullscreen(false);
    // TODO: implement exit fullscreen via Tauri command
  }, []);
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  {t(
                    "skill.githubUrlHint",
                    "Use the repository root URL. AgentForge will clone the repo and prefer SKILL.md, then README.md as fallback.",
                  )}
                </p>
                <div className="mt-3 rounded-lg border border-border bg-muted/20 p-3 text-xs text-muted-foreground space-y-1.5">
                  ...
                  <p>
                    {t(
                      ...
                      "If SKILL.md is missing, AgentForge will try README.md. Reinstall currently requires deleting the old copy first.",
                    )}
                  </p>
                </div>
              </div>
