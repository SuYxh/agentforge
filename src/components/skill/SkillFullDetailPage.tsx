...
export function SkillFullDetailPage() {
  ...
  const translationMode = "immersive" as const;
  ...
  useEffect(() => {
    ...
    async function resolveSkillMdContent() {
      ...
      try {
        const files = await skillApi.readLocalFiles(selectedSkill.id);
        const repoSkillMd =
          files.find(
            (file) =>
              !file.isDirectory &&
  ...
  const handleExport = async (format: "skillmd" | "json") => {
    ...
    try {
      let content: string;
      if (format === "json") {
        content = JSON.stringify(selectedSkill, null, 2);
      } else {
        const files = await skillApi.readLocalFiles(selectedSkill.id);
        const skillMd = files.find(
          (file) => !file.isDirectory && file.path.toLowerCase() === "skill.md",
        );
        content = skillMd?.content || selectedSkill.instructions || selectedSkill.content || "";
      }
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-background overflow-hidden animate-in fade-in slide-in-from-right-4 duration-300">
      ...
      {showBackToTop && activeTab !== "files" && (
        <button
          onClick={scrollToTop}
          className="absolute bottom-6 left-1/2 z-20 -translate-x-1/2 inline-flex items-center gap-2 rounded-full border border-border glass-strong px-4 py-2 text-sm font-medium text-foreground shadow-lg hover:bg-accent transition-colors"
        >
        ...
        message={
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground/80">
              {t(
                "skill.deleteHint",
                "仅从 AgentForge 库中移除，不会删除源文件目录。已分发到平台的安装会同时卸载。",
              )}
            </p>
          </div>
        }
