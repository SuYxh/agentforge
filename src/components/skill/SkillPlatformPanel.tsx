...
interface SkillPlatformPanelProps {
  ...
  detectedPlatformIds: Set<string>;
        ...
        {availablePlatforms.length > 0 && (
          ...
          <p className="text-[10px] text-muted-foreground leading-relaxed">
            {installMode === "copy"
              ? t(
                  "skill.copyModeDesc",
                  "Copy: Copies the SKILL.md file to each platform directory. Each copy is independent — edits in AgentForge won't sync automatically.",
                )
              : t(
                  "skill.symlinkDesc",
          ...
          <div className="space-y-2">
            {availablePlatforms.map((platform) => {
              ...
              const isDetected = detectedPlatformIds.has(platform.id);
              const isDisabled = !isDetected && !isInstalled;

              return (
                <div
                  key={platform.id}
                  onClick={() => {
                    if (isInstalled || isBatchInstalling || isDisabled) return;
                    togglePlatformSelection(platform.id);
                  }}
                  className={`p-3 rounded-xl border transition-all flex items-center justify-between ${
                    isDisabled
                      ? "bg-accent/20 border-border/50 opacity-50 cursor-not-allowed"
                      : isInstalled
                        ? "bg-primary/5 border-primary cursor-default"
                        : isSelected
                          ? "bg-primary/10 border-primary cursor-pointer"
                          : "bg-accent/30 border-border hover:bg-accent/50 cursor-pointer"
                  } ${isBatchInstalling && !isInstalled && !isDisabled ? "opacity-70 cursor-wait" : ""}`}
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-9 h-9 flex items-center justify-center flex-shrink-0 ${isDisabled ? "grayscale" : ""}`}>
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground">
                        {isDisabled
                          ? t("skill.notDetected", "Not detected")
                          : isInstalled
                            ? t("skill.installed")
                            : isSelected
