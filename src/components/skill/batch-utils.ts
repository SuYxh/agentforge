...
export function updateSkillTags(currentTags: string[] | undefined, tag: string, mode: SkillBatchTagMode): string[] {
...
export function collectSkillTags(skills: Skill[]): string[] {
  return Array.from(new Set(skills.flatMap((skill) => skill.tags || []).filter((tag) => tag.trim()))).sort((a, b) => a.localeCompare(b));
}
