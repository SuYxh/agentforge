...
import { SKILL_CATEGORIES } from '@/constants/skill-registry';
...
export function SkillStore() {
  ...
  const loadGitHubSkillRepo = useCallback(
    async (repoUrl: string): Promise<RegistrySkill[]> => {
      ...
      const repoMetaRaw = await skillApi.fetchRemoteContent(
      ...
      const remoteSkills = await Promise.all(
        skillFiles.map(async (item) => {
          ...
          let content: string;
          try {
            content = await skillApi.fetchRemoteContent(rawUrl);
          } catch {
            return null;
          }
  ...
  const loadMarketplaceStore = useCallback(
    async (
      ...
      const raw = await skillApi.fetchRemoteContent(resolvedUrl);
      if (!raw) return [];
      ...
      const mappedSkills = await Promise.all(
        directSkills.map(async (item: MarketplaceSkillEntry) => {
          ...
          if (!content && contentUrl) {
            try {
              content = await skillApi.fetchRemoteContent(contentUrl);
            } catch {
              content = '';
            }
          }
  ...
  const loadStoreSource = useCallback(
    async (sourceId: string, forceRefresh = false) => {
      ...
      const isFresh = cachedEntry && !cachedEntry.error && Date.now() - cachedEntry.loadedAt < REMOTE_STORE_TTL;
  ...
  const sourceMeta = useMemo(() => {
    ...
    if (selectedStoreSourceId === 'claude-code') {
      return {
        ...
        hint: t(
          ...
          '实时拉取 anthropics/skills 仓库，并按 AgentForge 的商店卡片结构展示。',
        ),
    ...
    return {
      ...
      hint: t('skill.storeHint', '从官方内置商店导入 AgentForge 已整理好的 skills。'),
      count: sourceRegistrySkills.length,
      showCatalog: true,
    };
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-background overflow-hidden">
      <div className="px-6 py-4 border-b border-border shrink-0 glass z-10 flex items-start justify-between gap-4">
